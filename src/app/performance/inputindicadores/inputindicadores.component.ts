...
export class InputindicadoresComponent implements OnInit {

  hoje = new Date();
  date: Date; 
  ontem = new Date(this.hoje.getFullYear(),this.hoje.getMonth(),this.hoje.getDate()-1)

  @Input() indicad : Indicadores
  referencia = new Date(this.hoje.getFullYear(),this.hoje.getMonth(),1)
  public refer = this.referencia.toISOString().substr(0,10)
  ...
  idSetor: number;
  i: number;
  classtemp: any[]= new Array();
  resumo: any[]= new Array();
  ...
  constructor(private messageService: MessageService, private performanceService: PerformanceService, private authService: AuthService ) { 
 ...
 // aqui  tem um bug na hora de 
  ...
  codSetor(dado){
    this.performanceService.gerencias().subscribe(
      response => {
  ...
  this.performanceService.indicadoresAtt(this.exeindicadorId,this.datareferencia, this.dataindicador, this.ciclo,  this.orcado, this.realizado,
    ...
    this.vlrretido,this.forecast2, this.forecast3)
   .subscribe(
       response => {
         if(response === null){
           //console.log("OK!!!!!!"),
