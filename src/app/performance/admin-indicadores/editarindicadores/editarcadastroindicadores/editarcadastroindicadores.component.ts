...
import { CadIndicador, campoDoGraficoId } from '../../admin-indicadores.model';
...
export class EditarcadastroindicadoresComponent implements OnInit {

  constructor(private adminindic: AdminIndicadoresService,  private messageService: MessageService,private performanceService: PerformanceService) {
  }

  listaIndicadores: CadIndicador[] = []

  FiltroListaGerencia
  FiltroGerenciaSelecionada: any;
  
  listaEdicaodeGraficoEixos
  listaEdicaodeGraficoTipos
  ...
  consultar(){
    ...
    this.adminindic.listaIndicadoresgerenc(this.FiltroGerenciaSelecionada.gerenciaId).subscribe(
      indicador  =>  {
  ...
  ajustarOrdem(){
    for(var i = 0; i < this.listaIndicadores.length; i++){
      ...
      this.adminindic.cadastroIndicadoresAtt(objetoDaVez).subscribe(
        indicador  =>  {
  ...
  indicadorSelecionado: CadIndicador;
...
u
  ...
  filtroGraficoData= new Date();
  filtroGraficoDataConvertida
  ...
  reloadArrays(){
    ...
    this.editarestilos=[]
    this.editarcores=[]
    this.adminindic.UnicoIndicadores(this.indicadorSelecionado.indicadorId).subscribe(
      indicador  =>  {
  ...
  SalvarTudo(indicadorSelec){
    ...
    this.adminindic.cadastroIndicadoresAtt(indicadorSelec).subscribe(
      indicador  =>  {
  ...
  //===============================================================================================
  //ALTERAR ORDEM
  alteraOrdemCampos: boolean = false
  campos
  ...
  SalvarCampoDoGrafico(id, campo){
    this.adminindic.atualizarCampos(campo, id).subscribe(
      indicador  =>  {
        this.messageService.add({severity: 'success', summary: 'Success', detail: 'Rotulo '+campo.label+' atualizado'});
      }
    );
  }
  deletarUmCampo: boolean = false

  deletarCampo(id){
    this.adminindic.deletarCampos(id).subscribe(
      indicador  =>  {
  ...
  atualizarCampo(i){
    ...
    console.log(this.NovoCampo)
    this.adminindic.atualizarCampos(this.NovoCampo,  this.indicadorSelecionado.campoDoGraficoId[i].campoDoGraficoId).subscribe(
      indicador  =>  {
    ...
    MontarGrafico(eixo, atributos){
      ...
      for(var i =0; i< this.indicadorSelecionado.campoDoGraficoId.length ; i++){
        dataset.push(
          this.indicadorSelecionado.campoDoGraficoId[i].estilo === 'Pontilhado' ?
          {
